...
import { restaurantsTypes } from "../types/loginTypes";
...
export const getRestaurantsAsync = (data) => {
    return async (dispatch) => {
        ...
        try {
            const rest = await filterCollection({
                key: data.key,
                collectionName: collectionsName.restaurants,
                value: data.value,
            });
            console.log(rest);
...
export const getMenuAsync = (data) => {
    return async (dispatch) => {
        ...
        try {
            dispatch(toggleLoading());
            const menu = await filterCollection({
                collectionName: collectionsName.menu,
                key: data.key,
                value: data.value,
            });
            if (data.type == null) {
                ...
                if (data.label == "type") {
                    filterMenu = menu.filter(
                        (item) => item[data.label] == data.type
                    );
